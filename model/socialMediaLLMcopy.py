...
class SocialMediaModel:
    ...
    def _clean(self):
        print("Initial shape:", self.properties_df.shape)
        
        # Drop unnecessary columns
        self.properties_df.drop(['CustomFolderName', 'CustomFileName', 'FileExtensionHandleMethod', 
                                'status_id', 'owner_screen_name', 'owner_display_name', 'media_urls'], 
                                axis=1, inplace=True)
        
        # Check the shape after dropping columns
        print("After dropping columns:", self.properties_df.shape)
        
        # Normalize media details
        ...
        print("After concatenation:", self.properties_df.shape)
        
        if 'type' not in self.properties_df.columns:
            self.properties_df['type'] = 0
        
        # Replaces image/video types
        self.properties_df['type'] = self.properties_df['type'].map({'image': 1, 'video': 2}).fillna(0)
        self.properties_df['is_retweet'] = self.properties_df['is_retweet'].map({'false': 0, 'true': 1}).fillna(0)
        
        # One-hot encoding for tweet_text
        ...
        # Drop the original tweet_text column
        ...
        # Drop missing values that might have been introduced
        # self.properties_df.dropna(inplace=True)
        
        # Check the shape after dropna
        print("After dropna:", self.properties_df.shape)
