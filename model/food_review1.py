...
class FoodReview1(db.Model):
    __tablename__ = 'food_reviews_1'
    ...
    def __repr__(self):
        return f"FoodReview1(id={self.id}, user_id={self.user_id}, food={self.food}, review={self.review}, rating={self.rating})"
    ...
    def restore(data):
        ...
        for food_data in data:
            ...
            food = FoodReview1.query.filter_by(food=food_name).first()
            if food:
                food.update(food_data)
            else:
                food = FoodReview1(**food_data)
                food.create()

def initFoodReviews1():
    """
    Initialize the FoodReview12345 table with default data.
    """
    reviews = [
        FoodReview1(user_id=1, food="Pizza", review="Delicious and cheesy!", rating="5"),
        FoodReview1(user_id=2, food="Burger", review="Juicy and filling.", rating="4"),
        FoodReview1(user_id=3, food="Sushi", review="Fresh and tasty.", rating="5"),
        FoodReview1(user_id=4, food="Pasta", review="Creamy and delightful.", rating="4.5"),
        FoodReview1(user_id=5, food="Salad", review="Healthy but could use more flavor.", rating="3")
    ]
